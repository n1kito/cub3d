...
void    render_player_dot_on_minimap(int *draw_end)
{
    ...
    dot_pos[0] = _map()->player_position[0] - (draw_end[0] - 10) * tile_size - 1;
    dot_pos[1] = _map()->player_position[1] - (draw_end[1] - 10) * tile_size- 1;
...
void    init_minimap_values(int *draw_pos, int *draw_end)
{
    ...
    if (_map()->player_position[0] / _map()->graphics->minimap_tile >= 5)
        draw_pos[0] = (_map()->player_position[0] / _map()->graphics->minimap_tile) - 5;
    if (_map()->player_position[1] / _map()->graphics->minimap_tile >= 5)
        draw_pos[1] = (_map()->player_position[1] / _map()->graphics->minimap_tile) - 5;
